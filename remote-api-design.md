...
トークン不一致時は401相当のレスポンス(`{ error: '...' }` かつ非2xxステータス)を返すこと。
...
2xx以外のステータスコードを返した場合、クライアントは
`外部DBへの通信に失敗しました(<status>)` として例外を投げる。
可能であればボディに `{ "error": "人が読める説明" }` を含めること(今回のクライアント実装では
ステータスコードのみ見るが、将来UI側でエラー内容を表示する拡張がしやすくなる)。
