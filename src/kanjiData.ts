/**
 * 学年別漢字配当表（2020年度改訂版、1026字）
 *
 * 出典: 文部科学省告示「小学校学習指導要領」別表（学年別漢字配当表）。
 * 2017年告示・2020年度全面実施の改訂版（都道府県名に必要な漢字20字を追加、計1026字）。
 * 各学年の文字数: 1年80字, 2年160字, 3年200字, 4年202字, 5年193字, 6年191字（合計1026字、重複なし）。
 */

export type Grade = 1 | 2 | 3 | 4 | 5 | 6;

export const GRADE_KANJI: Record<Grade, string[]> = {
...
// 漢字 → 学年 の逆引きマップ（モジュール読み込み時に構築）
const KANJI_TO_GRADE = new Map<string, Grade>();
for (const g of [1, 2, 3, 4, 5, 6] as Grade[]) {
  for (const ch of GRADE_KANJI[g]) {
    KANJI_TO_GRADE.set(ch, g);
  }
}

/** 学年別漢字配当表における学年を返す（配当表に含まれない漢字は null） */
export function kanjiGrade(char: string): Grade | null {
  return KANJI_TO_GRADE.get(char) ?? null;
}

/** 文字が漢字（CJK統合漢字）かどうかを判定する */
